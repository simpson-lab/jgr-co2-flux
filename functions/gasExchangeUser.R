...
gasExchangeUser <- function(temp, cond, ph, wind, altnotkpa = FALSE, salt = NULL, 
                             dic = NULL, alk = NULL, alt = NULL, kpa = NULL, pco2atm = NULL, trace = FALSE,
                             alknotdic = FALSE, diccalc = FALSE) { 
  
  if(is.null(c(dic)) & diccalc==FALSE) {
  ...
  if(is.null(salt)) {
    print("Assuming salt is 0")
  }
  if(is.null(kpa) & altnotkpa == FALSE) {
    stop("provide kpa")
  }
  if(is.null(alt) & altnotkpa == TRUE) {
